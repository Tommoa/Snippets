...
template<typename T> pair<T, bool> poly_winding(const vector<Point2D<T> >& poly, Point2D<T> pt) {
	int wn = 0;
...
template<typename T> T poly_area(const vector< Point2D<T> >& poly) { return poly_twice_area(poly) / 2;}
