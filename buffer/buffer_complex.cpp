...
void* Snippets::buffer_complex::allocate(size_t size) {
	if (memory_locations.begin() == memory_locations.end()&&max_size >= size) { // Nothing has been placed yet
		memory_locations.emplace(std::make_pair(0, size-1));
		return buf;
	}
	if (!memory_locations.empty() && max_size - ((--memory_locations.end())->second) > size) { // There is space at the end
	...
	size_t last = 0;
	for (auto itr = memory_locations.begin(); itr != memory_locations.end(); itr++) { // There is space between two other allocations (possibly due to a free)
...
void* Snippets::buffer_complex::malloc::reallocate(void* what, size_t size, void* worker) {
...
Snippets::buffer_complex::malloc Snippets::buffer_complex::default_allocator = Snippets::buffer_complex::malloc();

Snippets::buffer_complex::recursive Snippets::buffer_complex::recursive_allocator = Snippets::buffer_complex::recursive();
